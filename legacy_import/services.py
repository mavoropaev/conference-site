...
def import_users(rows):
    ...
    историю. Новым пользователям ставится непригодный для входа пароль: доступ восстанавливается
    через «забыли пароль» — функциональность внешняя по отношению к самому импорту.
    """
    User = get_user_model()
